...
module.exports = function (app) {
    const yt = {
        ...
        async download(queryOrYtUrl, userFormat = 'mp3') {
            ...
            if (search.p === 'search') {
                if (!search?.items?.length) throw Error(`Search results for ${queryOrYtUrl} not found`);
            ...
            if (convert.c_status === 'CONVERTING') {
                ...
                throw Error('File not ready / status unknown');
            } else {
                return convert;
            }
        },
    ...
    app.get('/downloader/ytdl', async (req, res) => {
        ...
        if (!format || !url) {
            return res.status(400).json({
                status: false,
                error: 'Parameters "format" and "url" are required',
            });
        }

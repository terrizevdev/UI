...
module.exports = function (app) {
  // Endpoint: /douyin/search?keyword=indonesia
  app.get('/douyin/search', async (req, res) => {
    const keyword = req.query.keyword;
    if (!keyword) {
      return res.status(400).json({
        status: false,
        message: 'Parameter ?keyword= must be filled'
      });
    }
    ...
    try {
      ...
      const headers = {
        ...
        // Replace cookie with valid session
      ...
      res.status(500).json({
        status: false,
        message: 'Failed to fetch data from Douyin',
        error: error.message
      });
    }
  });

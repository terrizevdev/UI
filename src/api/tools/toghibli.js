...
module.exports = function (app) {
  const ghibli = {
    ...
    getNewToken: async () => {
      try {
        ...
        if (res.status !== 200 || res.data?.status?.code !== '200')
          return { success: false, code: res.status || 500, result: { error: res.data?.status?.message || 'Failed to get token' } };
    ...
    generate: async ({ imageUrl }) => {
      if (!imageUrl) return { success: false, code: 400, result: { error: 'imageUrl cannot be empty' } };

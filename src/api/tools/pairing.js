...
module.exports = (app) => {
    ...
    /**
     * @api {get} /tools/V/pairing Get V Pairing Code
     * @apiName GetVPairingCode
     * @apiGroup Tools
     * @apiDescription Generate pairing code for a phone number using V service
     * 
     * @apiParam {String} number Phone number without + sign (required)
     * @apiParam {String} apikey Your API key (required)
     * 
     ...
     * @apiError {String} message Detailed error description
     */
    app.get('/tools/V/pairing', async (req, res) => {
        try {
            const { number } = req.query;
            ...
            // Make request to the V pairing service
            const pairingUrl = `https://vero-session-u8y9.onrender.com/pair?phone=${number}`;
            ...
            console.error('V Pairing code error:', error.message);
            ...
            if (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
                return res.status(503).json({
                    status: 503,
                    error: 'Service unavailable',
                    message: 'V Pairing service is currently unavailable'
                });
            }
            ...
            if (error.response) {
                ...
                return res.status(error.response.status).json({
                    status: error.response.status,
                    error: 'External service error',
                    message: error.response.data?.message || 'Error from V pairing service'
                });
            }
            ...
            if (error.request) {
                ...
                return res.status(504).json({
                    status: 504,
                    error: 'Gateway timeout',
                    message: 'No response received from V pairing service'
                });
            }
            ...
            return res.status(500).json({
                status: 500,
                error: 'Internal server error',
                message: 'Failed to generate pairing code from V service'
            });
        }
    });
};
